...
static int32_t ARM_SPI_Control(uint32_t control, uint32_t arg) {
    switch(control & ARM_SPI_CONTROL_Msk) {
        ...
        case ARM_SPI_MODE_MASTER:               // SPI Master (Output on MOSI, Input on MISO); arg =
                                                // Bus Speed in bps
            break;
...
ARM_DRIVER_SPI Driver_SPI0 = { ARM_SPI_GetVersion,   ARM_SPI_GetCapabilities, ARM_SPI_Initialize,
                               ARM_SPI_Uninitialize, ARM_SPI_PowerControl,    ARM_SPI_Send,
                               ARM_SPI_Receive,      ARM_SPI_Transfer,        ARM_SPI_GetDataCount,
                               ARM_SPI_Control,      ARM_SPI_GetStatus };
