 ...
 *    Replaced ARM_USBD_EndpointRead and ARM_USBD_EndpointWrite functions with
 * ARM_USBD_EndpointTransfer Added ARM_USBD_EndpointTransferGetResult function Renamed
 * ARM_USBD_EndpointAbort function to ARM_USBD_EndpointTransferAbort Changed prefix ARM_DRV ->
 * ARM_DRIVER Changed return values of some functions to int32_t Version 1.10 Namespace prefix ARM_
 * added Version 1.00 Initial release
 */
...
typedef void (*ARM_USBD_SignalDeviceEvent_t)(
    uint32_t event);               ///< Pointer to \ref ARM_USBD_SignalDeviceEvent : Signal USB
                                   ///< Device Event.
typedef void (*ARM_USBD_SignalEndpointEvent_t)(
    uint8_t ep_addr,
    uint32_t event);               ///< Pointer to \ref ARM_USBD_SignalEndpointEvent : Signal USB
                                   ///< Endpoint Event.
...
typedef struct _ARM_DRIVER_USBD {
    ...
    int32_t (*Initialize)(
        ...
        ARM_USBD_SignalEndpointEvent_t
            cb_endpoint_event);               ///< Pointer to \ref ARM_USBD_Initialize : Initialize
                                              ///< USB Device Interface.
    int32_t (*Uninitialize)(void);               ///< Pointer to \ref ARM_USBD_Uninitialize :
                                                 ///< De-initialize USB Device Interface.
    int32_t (*PowerControl)(
        ARM_POWER_STATE state);               ///< Pointer to \ref ARM_USBD_PowerControl : Control
    ...
    (void);                  ///< Pointer to \ref ARM_USBD_DeviceGetState : Get current USB Device
                             ///< State.
    int32_t (*DeviceRemoteWakeup)(
        void);               ///< Pointer to \ref ARM_USBD_DeviceRemoteWakeup : Trigger USB Remote
                             ///< Wakeup.
    int32_t (*DeviceSetAddress)(
        uint8_t dev_addr);               ///< Pointer to \ref ARM_USBD_DeviceSetAddress : Set USB
                                         ///< Device Address.
    int32_t (*ReadSetupPacket)(
        uint8_t * setup);                ///< Pointer to \ref ARM_USBD_ReadSetupPacket : Read setup
                                         ///< packet received over Control Endpoint.
    int32_t (*EndpointConfigure)(
        uint8_t ep_addr, uint8_t ep_type,
        uint16_t ep_max_packet_size);               ///< Pointer to \ref ARM_USBD_EndpointConfigure
                                                    ///< : Configure USB Endpoint.
    int32_t (*EndpointUnconfigure)(
        uint8_t ep_addr);               ///< Pointer to \ref ARM_USBD_EndpointUnconfigure :
                                        ///< Unconfigure USB Endpoint.
    int32_t (*EndpointStall)(uint8_t ep_addr,
                             bool stall);               ///< Pointer to \ref ARM_USBD_EndpointStall
                                                        ///< : Set/Clear Stall for USB Endpoint.
    int32_t (*EndpointTransfer)(
        uint8_t ep_addr, uint8_t * data,
        uint32_t num);               ///< Pointer to \ref ARM_USBD_EndpointTransfer : Read data from
                                     ///< or Write data to USB Endpoint.
    uint32_t (*EndpointTransferGetResult)(
        uint8_t ep_addr);               ///< Pointer to \ref ARM_USBD_EndpointTransferGetResult :
                                        ///< Get result of USB Endpoint transfer.
    int32_t (*EndpointTransferAbort)(
        uint8_t ep_addr);               ///< Pointer to \ref ARM_USBD_EndpointTransferAbort : Abort
                                        ///< current USB Endpoint transfer.
    uint16_t (*GetFrameNumber)(void);               ///< Pointer to \ref ARM_USBD_GetFrameNumber :
