...
#define ARM_SAI_CONFIGURE_TX                                                                       \
    (0x01UL)               ///< Configure Transmitter;  arg1 and arg2 provide additional
                           ///< configuration
#define ARM_SAI_CONFIGURE_RX                                                                       \
    (0x02UL)               ///< Configure Receiver;     arg1 and arg2 provide additional
                           ///< configuration
#define ARM_SAI_CONTROL_TX                                                                         \
    (0x03UL)               ///< Control Transmitter;    arg1.0: 0=disable (default), 1=enable;
                           ///< arg1.1: mute
#define ARM_SAI_CONTROL_RX                                                                         \
    (0x04UL)               ///< Control Receiver;       arg1.0: 0=disable (default), 1=enable
#define ARM_SAI_MASK_SLOTS_TX                                                                      \
    (0x05UL)               ///< Mask Transmitter slots; arg1 = mask (bit: 0=active, 1=inactive); all
                           ///< configured slots are active by default
#define ARM_SAI_MASK_SLOTS_RX                                                                      \
    (0x06UL)               ///< Mask Receiver    slots; arg1 = mask (bit: 0=active, 1=inactive); all
                           ///< configured slots are active by default
...
#define ARM_SAI_ASYNCHRONOUS                                                                       \
    (0UL << ARM_SAI_SYNCHRONIZATION_Pos)               ///< Asynchronous (default)
...
#define ARM_SAI_PROTOCOL_USER                                                                      \
    (0UL << ARM_SAI_PROTOCOL_Pos)                                        ///< User defined (default)
#define ARM_SAI_PROTOCOL_I2S (1UL << ARM_SAI_PROTOCOL_Pos)               ///< I2S
#define ARM_SAI_PROTOCOL_MSB_JUSTIFIED                                                             \
    (2UL << ARM_SAI_PROTOCOL_Pos)                                        ///< MSB (left) justified
#define ARM_SAI_PROTOCOL_LSB_JUSTIFIED                                                             \
    (3UL << ARM_SAI_PROTOCOL_Pos)                                        ///< LSB (right) justified
#define ARM_SAI_PROTOCOL_PCM_SHORT                                                                 \
    (4UL << ARM_SAI_PROTOCOL_Pos)                                        ///< PCM with short frame
#define ARM_SAI_PROTOCOL_PCM_LONG                                                                  \
    (5UL << ARM_SAI_PROTOCOL_Pos)                                        ///< PCM with long frame
...
#define ARM_SAI_DATA_SIZE(n)                                                                       \
    ((((n) -1UL) & 0x1FUL) << ARM_SAI_DATA_SIZE_Pos)               ///< Data size in bits (8..32)
...
#define ARM_SAI_MSB_FIRST                                                                          \
    (0UL << ARM_SAI_BIT_ORDER_Pos)               ///< Data is transferred with MSB first (default)
#define ARM_SAI_LSB_FIRST                                                                          \
    (1UL << ARM_SAI_BIT_ORDER_Pos)               ///< Data is transferred with LSB first; User
...
#define ARM_SAI_COMPANDING_NONE                                                                    \
    (0UL << ARM_SAI_COMPANDING_Pos)               ///< No companding (default)
...
#define ARM_SAI_CLOCK_POLARITY_0                                                                   \
    (0UL << ARM_SAI_CLOCK_POLARITY_Pos)               ///< Drive on falling edge, Capture on rising
                                                      ///< edge (default)
#define ARM_SAI_CLOCK_POLARITY_1                                                                   \
    (1UL << ARM_SAI_CLOCK_POLARITY_Pos)               ///< Drive on rising  edge, Capture on falling
                                                      ///< edge
...
#define ARM_SAI_MCLK_PIN_INACTIVE                                                                  \
    (0UL << ARM_SAI_MCLK_PIN_Pos)               ///< MCLK not used (default)
#define ARM_SAI_MCLK_PIN_OUTPUT                                                                    \
    (1UL << ARM_SAI_MCLK_PIN_Pos)               ///< MCLK is output (Master only)
#define ARM_SAI_MCLK_PIN_INPUT                                                                     \
    (2UL << ARM_SAI_MCLK_PIN_Pos)               ///< MCLK is input  (Master only)
...
#define ARM_SAI_FRAME_LENGTH(n)                                                                    \
    ((((n) -1UL) & 0x3FFUL)                                                                        \
     << ARM_SAI_FRAME_LENGTH_Pos)               ///< Frame length in bits (8..1024); default depends
                                                ///< on protocol and data
...
#define ARM_SAI_FRAME_SYNC_WIDTH(n)                                                                \
    ((((n) -1UL) & 0xFFUL)                                                                         \
     << ARM_SAI_FRAME_SYNC_WIDTH_Pos)               ///< Frame Sync width in bits (1..256);
                                                    ///< default=1; User Protocol only (ignored
                                                    ///< otherwise)
...
#define ARM_SAI_FRAME_SYNC_POLARITY_HIGH                                                           \
    (0UL                                                                                           \
     << ARM_SAI_FRAME_SYNC_POLARITY_Pos)               ///< Frame Sync is active high (default);
                                                       ///< User Protocol only (ignored otherwise)
#define ARM_SAI_FRAME_SYNC_POLARITY_LOW                                                            \
    (1UL << ARM_SAI_FRAME_SYNC_POLARITY_Pos)               ///< Frame Sync is active low; User
                                                           ///< Protocol only (ignored otherwise)

/*----- SAI Configuration (arg1): Frame Sync Early -----*/
#define ARM_SAI_FRAME_SYNC_EARLY                                                                   \
    (1UL << 19)               ///< Frame Sync one bit before the first bit of the frame; User
                              ///< Protocol only (ignored otherwise)
...
#define ARM_SAI_SLOT_COUNT(n)                                                                      \
    ((((n) -1UL) & 0x1FUL)                                                                         \
     << ARM_SAI_SLOT_COUNT_Pos)               ///< Number of slots in frame (1..32); default=1; User
                                              ///< Protocol only (ignored otherwise)

/*----- SAI Configuration (arg1): Slot Size -----*/
#define ARM_SAI_SLOT_SIZE_Pos 25
#define ARM_SAI_SLOT_SIZE_Msk (3UL << ARM_SAI_SLOT_SIZE_Pos)
#define ARM_SAI_SLOT_SIZE_DEFAULT                                                                  \
    (0UL << ARM_SAI_SLOT_SIZE_Pos)               ///< Slot size is equal to data size (default)
#define ARM_SAI_SLOT_SIZE_16                                                                       \
    (1UL << ARM_SAI_SLOT_SIZE_Pos)               ///< Slot size = 16 bits; User Protocol only
                                                 ///< (ignored otherwise)
#define ARM_SAI_SLOT_SIZE_32                                                                       \
    (3UL << ARM_SAI_SLOT_SIZE_Pos)               ///< Slot size = 32 bits; User Protocol only
                                                 ///< (ignored otherwise)
...
#define ARM_SAI_SLOT_OFFSET(n)                                                                     \
    (((n) &0x1FUL) << ARM_SAI_SLOT_OFFSET_Pos)               ///< Offset of first data bit in slot
                                                             ///< (0..31); default=0; User Protocol
...
/*----- SAI Control Codes: Configuration Parameters: Master Clock Prescaler (Master only and MCLK
 * Pin) -----*/
...
#define ARM_SAI_MCLK_PRESCALER(n)                                                                  \
    ((((n) -1UL) & 0xFFFUL)                                                                        \
     << ARM_SAI_MCLK_PRESCALER_Pos)               ///< MCLK prescaler; Audio_frequency = MCLK/n; n
                                                  ///< = 1..4096 (default=1)

/****** SAI specific error codes *****/
#define ARM_SAI_ERROR_SYNCHRONIZATION                                                              \
    (ARM_DRIVER_ERROR_SPECIFIC - 1)                ///< Specified Synchronization not supported
#define ARM_SAI_ERROR_PROTOCOL                                                                     \
    (ARM_DRIVER_ERROR_SPECIFIC - 2)                ///< Specified Protocol not supported
#define ARM_SAI_ERROR_DATA_SIZE                                                                    \
    (ARM_DRIVER_ERROR_SPECIFIC - 3)                ///< Specified Data size not supported
#define ARM_SAI_ERROR_BIT_ORDER                                                                    \
    (ARM_DRIVER_ERROR_SPECIFIC - 4)                ///< Specified Bit order not supported
#define ARM_SAI_ERROR_MONO_MODE                                                                    \
    (ARM_DRIVER_ERROR_SPECIFIC - 5)                ///< Specified Mono mode not supported
#define ARM_SAI_ERROR_COMPANDING                                                                   \
    (ARM_DRIVER_ERROR_SPECIFIC - 6)                ///< Specified Companding not supported
#define ARM_SAI_ERROR_CLOCK_POLARITY                                                               \
    (ARM_DRIVER_ERROR_SPECIFIC - 7)                ///< Specified Clock polarity not supported
#define ARM_SAI_ERROR_AUDIO_FREQ                                                                   \
    (ARM_DRIVER_ERROR_SPECIFIC - 8)                ///< Specified Audio frequency not supported
#define ARM_SAI_ERROR_MCLK_PIN                                                                     \
    (ARM_DRIVER_ERROR_SPECIFIC - 9)                ///< Specified MCLK Pin setting not supported
#define ARM_SAI_ERROR_MCLK_PRESCALER                                                               \
    (ARM_DRIVER_ERROR_SPECIFIC - 10)               ///< Specified MCLK Prescaler not supported
#define ARM_SAI_ERROR_FRAME_LENGTH                                                                 \
    (ARM_DRIVER_ERROR_SPECIFIC - 11)               ///< Specified Frame length not supported
#define ARM_SAI_ERROR_FRAME_LENGHT                                                                 \
    (ARM_DRIVER_ERROR_SPECIFIC -                                                                   \
     11)               ///< Specified Frame length not supported @deprecated use \ref
                       ///< ARM_SAI_ERROR_FRAME_LENGTH instead
#define ARM_SAI_ERROR_FRAME_SYNC_WIDTH                                                             \
    (ARM_DRIVER_ERROR_SPECIFIC - 12)               ///< Specified Frame Sync width not supported
...
#define ARM_SAI_ERROR_SLOT_COUNT                                                                   \
    (ARM_DRIVER_ERROR_SPECIFIC - 15)               ///< Specified Slot count not supported
#define ARM_SAI_ERROR_SLOT_SIZE                                                                    \
    (ARM_DRIVER_ERROR_SPECIFIC - 16)               ///< Specified Slot size not supported
#define ARM_SAI_ERROR_SLOT_OFFESET                                                                 \
    (ARM_DRIVER_ERROR_SPECIFIC - 17)               ///< Specified Slot offset not supported
...
typedef struct _ARM_SAI_STATUS {
    ...
    uint32_t tx_underflow : 1;               ///< Transmit data underflow detected (cleared on start
                                             ///< of next send operation)
    uint32_t rx_overflow : 1;               ///< Receive data overflow detected (cleared on start of
                                            ///< next receive operation)
    uint32_t frame_error : 1;               ///< Sync Frame error detected (cleared on start of next
...
#define ARM_SAI_EVENT_FRAME_ERROR                                                                  \
    (1UL << 4)               ///< Sync Frame error in Slave mode (optional)
...
typedef void (*ARM_SAI_SignalEvent_t)(
    uint32_t event);               ///< Pointer to \ref ARM_SAI_SignalEvent : Signal SAI Event.
...
typedef struct _ARM_SAI_CAPABILITIES {
    ...
    uint32_t event_frame_error : 1;                ///< supports Frame error event: \ref
...
typedef struct _ARM_DRIVER_SAI {
    ...
    int32_t (*Initialize)(
        ARM_SAI_SignalEvent_t cb_event);               ///< Pointer to \ref ARM_SAI_Initialize :
                                                       ///< Initialize SAI Interface.
    int32_t (*Uninitialize)(void);                     ///< Pointer to \ref ARM_SAI_Uninitialize :
                                                       ///< De-initialize SAI Interface.
    int32_t (*PowerControl)(
        ARM_POWER_STATE state);                  ///< Pointer to \ref ARM_SAI_PowerControl : Control
                                                 ///< SAI Interface Power.
    int32_t (*Send)(const void * data,
                    uint32_t num);               ///< Pointer to \ref ARM_SAI_Send : Start sending
                                                 ///< data to SAI Interface.
    int32_t (*Receive)(void * data,
                       uint32_t num);               ///< Pointer to \ref ARM_SAI_Receive : Start
                                                    ///< receiving data from SAI Interface.
    uint32_t (*GetTxCount)(
    ...
    int32_t (*Control)(
        uint32_t control, uint32_t arg1,
