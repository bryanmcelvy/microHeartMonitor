...
#define ARM_MCI_RESPONSE_NONE                                                                      \
    (0UL << ARM_MCI_RESPONSE_Pos)               ///< No response expected (default)
#define ARM_MCI_RESPONSE_SHORT                                                                     \
    (1UL << ARM_MCI_RESPONSE_Pos)               ///< Short response (48-bit)
#define ARM_MCI_RESPONSE_SHORT_BUSY                                                                \
    (2UL << ARM_MCI_RESPONSE_Pos)               ///< Short response with busy signal (48-bit)
#define ARM_MCI_RESPONSE_LONG                                                                      \
    (3UL << ARM_MCI_RESPONSE_Pos)               ///< Long response (136-bit)
...
#define ARM_MCI_CARD_INITIALIZE                                                                    \
    (1UL << 6)               ///< Execute Memory Card initialization sequence

#define ARM_MCI_INTERRUPT_COMMAND                                                                  \
    (1UL << 7)               ///< Send Interrupt command (CMD40 - MMC only)
#define ARM_MCI_INTERRUPT_RESPONSE                                                                 \
    (1UL << 8)               ///< Send Interrupt response (CMD40 - MMC only)

#define ARM_MCI_BOOT_OPERATION (1UL << 9)               ///< Execute Boot operation (MMC only)
#define ARM_MCI_BOOT_ALTERNATIVE                                                                   \
    (1UL << 10)               ///< Execute Alternative Boot operation (MMC only)
#define ARM_MCI_BOOT_ACK (1UL << 11)               ///< Expect Boot Acknowledge (MMC only)

#define ARM_MCI_CCSD                                                                               \
    (1UL << 12)               ///< Send Command Completion Signal Disable (CCSD) for CE-ATA device
#define ARM_MCI_CCS                                                                                \
    (1UL << 13)               ///< Expect Command Completion Signal (CCS) for CE-ATA device
...
/****** MCI Control Codes *****/
#define ARM_MCI_BUS_SPEED                                                                          \
    (0x01UL)               ///< Set Bus Speed; arg = requested speed in bits/s; returns configured
                           ///< speed in bits/s
...
#define ARM_MCI_DRIVER_STRENGTH                                                                    \
    (0x05UL)               ///< Set SD UHS-I Driver Strength as specified with arg
#define ARM_MCI_CONTROL_RESET                                                                      \
    (0x06UL)               ///< Control optional RST_n Pin (eMMC); arg: 0=inactive, 1=active
#define ARM_MCI_CONTROL_CLOCK_IDLE                                                                 \
    (0x07UL)               ///< Control Clock generation on CLK Pin when idle; arg: 0=disabled,
                           ///< 1=enabled
#define ARM_MCI_UHS_TUNING_OPERATION                                                               \
    (0x08UL)               ///< Sampling clock Tuning operation (SD UHS-I); arg: 0=reset, 1=execute
#define ARM_MCI_UHS_TUNING_RESULT                                                                  \
    (0x09UL)               ///< Sampling clock Tuning result (SD UHS-I); returns: 0=done, 1=in
                           ///< progress, -1=error
#define ARM_MCI_DATA_TIMEOUT                                                                       \
    (0x0AUL)               ///< Set Data timeout; arg = timeout in bus cycles
#define ARM_MCI_CSS_TIMEOUT                                                                        \
    (0x0BUL)               ///< Set Command Completion Signal (CCS) timeout; arg = timeout in bus
                           ///< cycles
#define ARM_MCI_MONITOR_SDIO_INTERRUPT                                                             \
    (0x0CUL)               ///< Monitor SD I/O interrupt: arg: 0=disabled, 1=enabled
#define ARM_MCI_CONTROL_READ_WAIT                                                                  \
    (0x0DUL)               ///< Control Read/Wait for SD I/O; arg: 0=disabled, 1=enabled
#define ARM_MCI_SUSPEND_TRANSFER                                                                   \
    (0x0EUL)               ///< Suspend Data transfer (SD I/O); returns number of remaining bytes to
                           ///< transfer
#define ARM_MCI_RESUME_TRANSFER (0x0FUL)               ///< Resume Data transfer (SD I/O)

/*----- MCI Bus Speed Mode -----*/
#define ARM_MCI_BUS_DEFAULT_SPEED                                                                  \
    (0x00UL)                                          ///< SD/MMC: Default Speed mode up to 25/26MHz
#define ARM_MCI_BUS_HIGH_SPEED (0x01UL)               ///< SD/MMC: High    Speed mode up to 50/52MHz
#define ARM_MCI_BUS_UHS_SDR12                                                                      \
    (0x02UL)               ///< SD: SDR12  (Single Data Rate) up to  25MHz,  12.5MB/s: UHS-I (Ultra
                           ///< High Speed) 1.8V signaling
#define ARM_MCI_BUS_UHS_SDR25                                                                      \
    (0x03UL)               ///< SD: SDR25  (Single Data Rate) up to  50MHz,  25  MB/s: UHS-I (Ultra
                           ///< High Speed) 1.8V signaling
#define ARM_MCI_BUS_UHS_SDR50                                                                      \
    (0x04UL)               ///< SD: SDR50  (Single Data Rate) up to 100MHz,  50  MB/s: UHS-I (Ultra
                           ///< High Speed) 1.8V signaling
#define ARM_MCI_BUS_UHS_SDR104                                                                     \
    (0x05UL)               ///< SD: SDR104 (Single Data Rate) up to 208MHz, 104  MB/s: UHS-I (Ultra
                           ///< High Speed) 1.8V signaling
#define ARM_MCI_BUS_UHS_DDR50                                                                      \
    (0x06UL)               ///< SD: DDR50  (Dual Data Rate)   up to  50MHz,  50  MB/s: UHS-I (Ultra
                           ///< High Speed) 1.8V signaling
...
#define ARM_MCI_BUS_DATA_WIDTH_4_DDR                                                               \
    (0x03UL)               ///< Bus data width: 4 bits, DDR (Dual Data Rate) - MMC only
#define ARM_MCI_BUS_DATA_WIDTH_8_DDR                                                               \
    (0x04UL)               ///< Bus data width: 8 bits, DDR (Dual Data Rate) - MMC only
...
#define ARM_MCI_POWER_VDD_OFF                                                                      \
    (0x01UL << ARM_MCI_POWER_VDD_Pos)               ///< VDD (VCC) turned off
...
#define ARM_MCI_POWER_VCCQ_OFF                                                                     \
    (0x01UL << ARM_MCI_POWER_VCCQ_Pos)               ///< eMMC VCCQ turned off
#define ARM_MCI_POWER_VCCQ_3V3                                                                     \
    (0x02UL << ARM_MCI_POWER_VCCQ_Pos)               ///< eMMC VCCQ = 3.3V
#define ARM_MCI_POWER_VCCQ_1V8                                                                     \
    (0x03UL << ARM_MCI_POWER_VCCQ_Pos)               ///< eMMC VCCQ = 1.8V
#define ARM_MCI_POWER_VCCQ_1V2                                                                     \
    (0x04UL << ARM_MCI_POWER_VCCQ_Pos)               ///< eMMC VCCQ = 1.2V
...
typedef struct _ARM_MCI_STATUS {
    ...
    uint32_t command_timeout : 1;               ///< Command timeout flag (cleared on start of next
                                                ///< command)
    uint32_t
        command_error : 1;               ///< Command error flag (cleared on start of next command)
    uint32_t transfer_active : 1;                ///< Transfer active flag
    uint32_t transfer_timeout : 1;               ///< Transfer timeout flag (cleared on start of
                                                 ///< next command)
    uint32_t transfer_error : 1;                 ///< Transfer error flag (cleared on start of next
                                                 ///< command)
    uint32_t sdio_interrupt : 1;                 ///< SD I/O Interrupt flag (cleared on start of
                                                 ///< monitoring)
...
#define ARM_MCI_EVENT_COMMAND_ERROR                                                                \
    (1UL << 4)               ///< Command response error (CRC error or invalid response)
...
#define ARM_MCI_EVENT_CCS_TIMEOUT                                                                  \
    (1UL << 10)               ///< Command Completion Signal (CCS) Timeout
...
typedef struct _ARM_MCI_CAPABILITIES {
    ...
    uint32_t data_width_4_ddr : 1;               ///< Supports 4-bit data, DDR (Dual Data Rate) -
                                                 ///< MMC only
    uint32_t data_width_8_ddr : 1;               ///< Supports 8-bit data, DDR (Dual Data Rate) -
                                                 ///< MMC only
    uint32_t high_speed : 1;                     ///< Supports SD/MMC High Speed Mode
    uint32_t
        uhs_signaling : 1;                 ///< Supports SD UHS-I (Ultra High Speed) 1.8V signaling
    uint32_t uhs_tuning : 1;               ///< Supports SD UHS-I tuning
    uint32_t
        uhs_sdr50 : 1;                ///< Supports SD UHS-I SDR50  (Single Data Rate) up to  50MB/s
    uint32_t
        uhs_sdr104 : 1;               ///< Supports SD UHS-I SDR104 (Single Data Rate) up to 104MB/s
    uint32_t
        uhs_ddr50 : 1;                ///< Supports SD UHS-I DDR50  (Dual   Data Rate) up to  50MB/s
    ...
    uint32_t ccs_timeout : 1;               ///< Supports Command Completion Signal (CCS) timeout
                                            ///< for CE-ATA
...
typedef struct _ARM_DRIVER_MCI {
    ...
    int32_t (*Initialize)(
        ARM_MCI_SignalEvent_t cb_event);               ///< Pointer to \ref ARM_MCI_Initialize :
                                                       ///< Initialize MCI Interface.
    int32_t (*Uninitialize)(void);                     ///< Pointer to \ref ARM_MCI_Uninitialize :
                                                       ///< De-initialize MCI Interface.
    int32_t (*PowerControl)(
        ARM_POWER_STATE state);               ///< Pointer to \ref ARM_MCI_PowerControl : Control
                                              ///< MCI Interface Power.
    int32_t (*CardPower)(uint32_t voltage);               ///< Pointer to \ref ARM_MCI_CardPower :
                                                          ///< Set card power supply voltage.
    int32_t (*ReadCD)(
    ...
    int32_t (*SendCommand)(
        uint32_t cmd, uint32_t arg, uint32_t flags,
        uint32_t * response);               ///< Pointer to \ref ARM_MCI_SendCommand : Send Command
                                            ///< to card and get the response.
    int32_t (*SetupTransfer)(
        uint8_t * data, uint32_t block_count, uint32_t block_size,
        uint32_t mode);                     ///< Pointer to \ref ARM_MCI_SetupTransfer : Setup data
                                            ///< transfer operation.
    int32_t (*AbortTransfer)(void);               ///< Pointer to \ref ARM_MCI_AbortTransfer : Abort
