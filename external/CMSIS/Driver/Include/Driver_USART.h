...
#define ARM_USART_MODE_ASYNCHRONOUS                                                                \
    (0x01UL << ARM_USART_CONTROL_Pos)               ///< UART (Asynchronous); arg = Baudrate
#define ARM_USART_MODE_SYNCHRONOUS_MASTER                                                          \
    (0x02UL << ARM_USART_CONTROL_Pos)               ///< Synchronous Master (generates clock
                                                    ///< signal); arg = Baudrate
...
#define ARM_USART_MODE_SINGLE_WIRE                                                                 \
    (0x04UL                                                                                        \
     << ARM_USART_CONTROL_Pos)               ///< UART Single-wire (half-duplex); arg = Baudrate
#define ARM_USART_MODE_IRDA                                                                        \
    (0x05UL << ARM_USART_CONTROL_Pos)               ///< UART IrDA; arg = Baudrate
#define ARM_USART_MODE_SMART_CARD                                                                  \
    (0x06UL << ARM_USART_CONTROL_Pos)               ///< UART Smart Card; arg = Baudrate
...
#define ARM_USART_DATA_BITS_8                                                                      \
    (0UL << ARM_USART_DATA_BITS_Pos)               ///< 8 Data bits (default)
...
#define ARM_USART_STOP_BITS_1                                                                      \
    (0UL << ARM_USART_STOP_BITS_Pos)               ///< 1 Stop bit (default)
...
#define ARM_USART_FLOW_CONTROL_NONE                                                                \
    (0UL << ARM_USART_FLOW_CONTROL_Pos)               ///< No Flow Control (default)
#define ARM_USART_FLOW_CONTROL_RTS                                                                 \
    (1UL << ARM_USART_FLOW_CONTROL_Pos)               ///< RTS Flow Control
#define ARM_USART_FLOW_CONTROL_CTS                                                                 \
    (2UL << ARM_USART_FLOW_CONTROL_Pos)               ///< CTS Flow Control
#define ARM_USART_FLOW_CONTROL_RTS_CTS                                                             \
    (3UL << ARM_USART_FLOW_CONTROL_Pos)               ///< RTS/CTS Flow Control
...
#define ARM_USART_SET_DEFAULT_TX_VALUE                                                             \
    (0x10UL << ARM_USART_CONTROL_Pos)               ///< Set default Transmit value (Synchronous
                                                    ///< Receive only); arg = value
#define ARM_USART_SET_IRDA_PULSE                                                                   \
    (0x11UL                                                                                        \
     << ARM_USART_CONTROL_Pos)               ///< Set IrDA Pulse in ns; arg: 0=3/16 of bit period
#define ARM_USART_SET_SMART_CARD_GUARD_TIME                                                        \
    (0x12UL << ARM_USART_CONTROL_Pos)               ///< Set Smart Card Guard Time; arg = number of
                                                    ///< bit periods
#define ARM_USART_SET_SMART_CARD_CLOCK                                                             \
    (0x13UL << ARM_USART_CONTROL_Pos)               ///< Set Smart Card Clock in Hz; arg: 0=Clock
                                                    ///< not generated
#define ARM_USART_CONTROL_SMART_CARD_NACK                                                          \
    (0x14UL << ARM_USART_CONTROL_Pos)               ///< Smart Card NACK generation; arg:
                                                    ///< 0=disabled, 1=enabled
#define ARM_USART_CONTROL_TX                                                                       \
    (0x15UL << ARM_USART_CONTROL_Pos)               ///< Transmitter; arg: 0=disabled, 1=enabled
#define ARM_USART_CONTROL_RX                                                                       \
    (0x16UL << ARM_USART_CONTROL_Pos)               ///< Receiver; arg: 0=disabled, 1=enabled
#define ARM_USART_CONTROL_BREAK                                                                    \
    (0x17UL << ARM_USART_CONTROL_Pos)               ///< Continuous Break transmission; arg:
                                                    ///< 0=disabled, 1=enabled
#define ARM_USART_ABORT_SEND                                                                       \
    (0x18UL << ARM_USART_CONTROL_Pos)               ///< Abort \ref ARM_USART_Send
#define ARM_USART_ABORT_RECEIVE                                                                    \
    (0x19UL << ARM_USART_CONTROL_Pos)               ///< Abort \ref ARM_USART_Receive
#define ARM_USART_ABORT_TRANSFER                                                                   \
    (0x1AUL << ARM_USART_CONTROL_Pos)               ///< Abort \ref ARM_USART_Transfer

/****** USART specific error codes *****/
#define ARM_USART_ERROR_MODE                                                                       \
    (ARM_DRIVER_ERROR_SPECIFIC - 1)               ///< Specified Mode not supported
#define ARM_USART_ERROR_BAUDRATE                                                                   \
    (ARM_DRIVER_ERROR_SPECIFIC - 2)               ///< Specified baudrate not supported
#define ARM_USART_ERROR_DATA_BITS                                                                  \
    (ARM_DRIVER_ERROR_SPECIFIC - 3)               ///< Specified number of Data bits not supported
#define ARM_USART_ERROR_PARITY                                                                     \
    (ARM_DRIVER_ERROR_SPECIFIC - 4)               ///< Specified Parity not supported
#define ARM_USART_ERROR_STOP_BITS                                                                  \
    (ARM_DRIVER_ERROR_SPECIFIC - 5)               ///< Specified number of Stop bits not supported
#define ARM_USART_ERROR_FLOW_CONTROL                                                               \
    (ARM_DRIVER_ERROR_SPECIFIC - 6)               ///< Specified Flow Control not supported
#define ARM_USART_ERROR_CPOL                                                                       \
    (ARM_DRIVER_ERROR_SPECIFIC - 7)               ///< Specified Clock Polarity not supported
#define ARM_USART_ERROR_CPHA                                                                       \
    (ARM_DRIVER_ERROR_SPECIFIC - 8)               ///< Specified Clock Phase not supported
...
typedef struct _ARM_USART_STATUS {
    ...
    uint32_t rx_break : 1;                  ///< Break detected on receive (cleared on start of next
                                            ///< receive operation)
    uint32_t rx_framing_error : 1;               ///< Framing error detected on receive (cleared on
                                                 ///< start of next receive operation)
    uint32_t rx_parity_error : 1;                ///< Parity error detected on receive (cleared on
                                                 ///< start of next receive operation)
...
/****** USART Event *****/
#define ARM_USART_EVENT_SEND_COMPLETE                                                              \
    (1UL << 0)               ///< Send completed; however USART may still transmit data
...
#define ARM_USART_EVENT_TX_UNDERFLOW                                                               \
    (1UL << 4)               ///< Transmit data not available (Synchronous Slave)
#define ARM_USART_EVENT_RX_OVERFLOW (1UL << 5)               ///< Receive data overflow
#define ARM_USART_EVENT_RX_TIMEOUT                                                                 \
    (1UL << 6)                                            ///< Receive character timeout (optional)
#define ARM_USART_EVENT_RX_BREAK (1UL << 7)               ///< Break detected on receive
#define ARM_USART_EVENT_RX_FRAMING_ERROR                                                           \
    (1UL << 8)                                            ///< Framing error detected on receive
#define ARM_USART_EVENT_RX_PARITY_ERROR                                                            \
    (1UL << 9)                                            ///< Parity error detected on receive
...
typedef struct _ARM_USART_CAPABILITIES {
    ...
    uint32_t event_rx_timeout : 1;                 ///< Signal receive character timeout event: \ref
...
typedef struct _ARM_DRIVER_USART {
    ...
    int32_t (*Initialize)(
        ARM_USART_SignalEvent_t cb_event);               ///< Pointer to \ref ARM_USART_Initialize :
                                                         ///< Initialize USART Interface.
    int32_t (*Uninitialize)(void);               ///< Pointer to \ref ARM_USART_Uninitialize :
                                                 ///< De-initialize USART Interface.
    int32_t (*PowerControl)(
        ARM_POWER_STATE state);               ///< Pointer to \ref ARM_USART_PowerControl : Control
                                              ///< USART Interface Power.
    int32_t (*Send)(const void * data,
                    uint32_t num);               ///< Pointer to \ref ARM_USART_Send : Start sending
                                                 ///< data to USART transmitter.
    int32_t (*Receive)(void * data,
                       uint32_t num);                ///< Pointer to \ref ARM_USART_Receive : Start
                                                     ///< receiving data from USART receiver.
    int32_t (*Transfer)(const void * data_out, void * data_in,
                        uint32_t num);               ///< Pointer to \ref ARM_USART_Transfer : Start
                                                     ///< sending/receiving data to/from USART.
    uint32_t (*GetTxCount)(
    ...
    int32_t (*Control)(uint32_t control,
                       uint32_t arg);               ///< Pointer to \ref ARM_USART_Control : Control
                                                    ///< USART Interface.
    ARM_USART_STATUS (*GetStatus)
    (void);                         ///< Pointer to \ref ARM_USART_GetStatus : Get USART status.
    int32_t (*SetModemControl)(
        ARM_USART_MODEM_CONTROL
            control);               ///< Pointer to \ref ARM_USART_SetModemControl : Set USART Modem
                                    ///< Control line state.
    ARM_USART_MODEM_STATUS (*GetModemStatus)
    (void);               ///< Pointer to \ref ARM_USART_GetModemStatus : Get USART Modem Status
                          ///< lines state.
