...
typedef enum _ARM_CAN_BITRATE_SELECT {
    ARM_CAN_BITRATE_NOMINAL,               ///< Select nominal (flexible data-rate arbitration)
                                           ///< bitrate
...
typedef enum _ARM_CAN_OBJ_CONFIG {
    ...
    ARM_CAN_OBJ_RX_RTR_TX_DATA,               ///< CAN object that on RTR reception automatically
                                              ///< transmits Data Frame
    ARM_CAN_OBJ_TX_RTR_RX_DATA                ///< CAN object that transmits RTR and automatically
                                              ///< receives Data Frame
} ARM_CAN_OBJ_CONFIG;
...
typedef struct _ARM_CAN_OBJ_CAPABILITIES {
    ...
    uint32_t rx_rtr_tx_data : 1;               ///< Object supports RTR reception and automatic Data
                                               ///< Frame transmission
    uint32_t tx_rtr_rx_data : 1;               ///< Object supports RTR transmission and automatic
                                               ///< Data Frame reception
    uint32_t
        multiple_filters : 1;               ///< Object allows assignment of multiple filters to it
...
#define ARM_CAN_SET_FD_MODE                                                                        \
    (1UL << ARM_CAN_CONTROL_Pos)               ///< Set FD operation mode;                   arg: 0
                                               ///< = disable, 1 = enable
#define ARM_CAN_ABORT_MESSAGE_SEND                                                                 \
    (2UL << ARM_CAN_CONTROL_Pos)               ///< Abort sending of CAN message;            arg =
                                               ///< object
#define ARM_CAN_CONTROL_RETRANSMISSION                                                             \
    (3UL << ARM_CAN_CONTROL_Pos)               ///< Enable/disable automatic retransmission; arg: 0
                                               ///< = disable, 1 = enable (default state)
#define ARM_CAN_SET_TRANSCEIVER_DELAY                                                              \
    (4UL << ARM_CAN_CONTROL_Pos)               ///< Set transceiver delay;                   arg =
                                               ///< delay in time quanta
...
#define ARM_CAN_STANDARD_ID(id)                                                                    \
    (id & 0x000007FFUL)                ///< CAN identifier in standard format (11-bits)
#define ARM_CAN_EXTENDED_ID(id)                                                                    \
    ((id & 0x1FFFFFFFUL) |                                                                         \
     ARM_CAN_ID_IDE_Msk)               ///< CAN identifier in extended format (29-bits)
...
#define ARM_CAN_INVALID_BITRATE_SELECT                                                             \
    (ARM_DRIVER_ERROR_SPECIFIC - 1)               ///< Bitrate selection not supported
#define ARM_CAN_INVALID_BITRATE                                                                    \
    (ARM_DRIVER_ERROR_SPECIFIC - 2)               ///< Requested bitrate not supported
#define ARM_CAN_INVALID_BIT_PROP_SEG                                                               \
    (ARM_DRIVER_ERROR_SPECIFIC - 3)               ///< Propagation segment value not supported
#define ARM_CAN_INVALID_BIT_PHASE_SEG1                                                             \
    (ARM_DRIVER_ERROR_SPECIFIC - 4)               ///< Phase segment 1 value not supported
#define ARM_CAN_INVALID_BIT_PHASE_SEG2                                                             \
    (ARM_DRIVER_ERROR_SPECIFIC - 5)               ///< Phase segment 2 value not supported
#define ARM_CAN_INVALID_BIT_SJW                                                                    \
    (ARM_DRIVER_ERROR_SPECIFIC - 6)               ///< SJW value not supported
#define ARM_CAN_NO_MESSAGE_AVAILABLE                                                               \
    (ARM_DRIVER_ERROR_SPECIFIC - 7)               ///< Message is not available

/****** CAN Status codes *****/
#define ARM_CAN_UNIT_STATE_INACTIVE                                                                \
    (0U)               ///< Unit state: Not active on bus (initialization)
#define ARM_CAN_UNIT_STATE_ACTIVE                                                                  \
    (1U)               ///< Unit state: Active on bus (can generate active error frame)
#define ARM_CAN_UNIT_STATE_PASSIVE                                                                 \
    (2U)               ///< Unit state: Error passive (can not generate active error frame)
#define ARM_CAN_UNIT_STATE_BUS_OFF                                                                 \
    (3U)               ///< Unit state: Bus-off (can recover to active state)
...
#define ARM_CAN_EVENT_UNIT_WARNING                                                                 \
    (2U)               ///< Unit entered Error Warning state (one or both error counters >= 96)
...
/**
  ...
  \fn          int32_t ARM_CAN_SetBitrate (ARM_CAN_BITRATE_SELECT select, uint32_t bitrate, uint32_t
  bit_segments) \brief       Set bitrate for CAN interface. \param[in]   select       Bitrate
  selection
  ...
  \param[in]   bit_segments Bit segments settings
                 - \ref ARM_CAN_BIT_PROP_SEG(x) :   number of time quanta for propagation time
  segment
                 ...
                 - \ref ARM_CAN_BIT_SJW(x) :        number of time quanta for (re-)synchronization
  jump width \return      \ref execution_status
  ...
  \fn          int32_t ARM_CAN_ObjectSetFilter (uint32_t obj_idx, ARM_CAN_FILTER_OPERATION
  operation, uint32_t id, uint32_t arg) \brief       Add or remove filter for message reception.
  ...
  \param[in]   obj_cfg  Object configuration state
                 ...
                 - \ref ARM_CAN_OBJ_RX_RTR_TX_DATA : configure object that on RTR reception
  automatically transmits Data Frame
                 - \ref ARM_CAN_OBJ_TX_RTR_RX_DATA : configure object that transmits RTR and
  automatically receives Data Frame \return      \ref execution_status

  \fn          int32_t ARM_CAN_MessageSend (uint32_t obj_idx, ARM_CAN_MSG_INFO *msg_info, const
  uint8_t *data, uint8_t size) \brief       Send message on CAN bus. \param[in]   obj_idx  Object
  index \param[in]   msg_info Pointer to CAN message information \param[in]   data     Pointer to
  data buffer \param[in]   size     Number of data bytes to send \return      value >= 0  number of
  data bytes accepted to send \return      value < 0   \ref execution_status

  \fn          int32_t ARM_CAN_MessageRead (uint32_t obj_idx, ARM_CAN_MSG_INFO *msg_info, uint8_t
  *data, uint8_t size) \brief       Read message received on CAN bus. \param[in]   obj_idx  Object
  index \param[out]  msg_info Pointer to read CAN message information \param[out]  data     Pointer
  to data buffer for read data \param[in]   size     Maximum number of data bytes to read \return
  value >= 0  number of data bytes read \return      value < 0   \ref execution_status
...
typedef void (*ARM_CAN_SignalUnitEvent_t)(
    uint32_t event);               ///< Pointer to \ref ARM_CAN_SignalUnitEvent   : Signal CAN Unit
                                   ///< Event.
typedef void (*ARM_CAN_SignalObjectEvent_t)(
    uint32_t obj_idx, uint32_t event);               ///< Pointer to \ref ARM_CAN_SignalObjectEvent
...
typedef struct _ARM_CAN_CAPABILITIES {
    ...
    uint32_t
        reentrant_operation : 1;               ///< Support for reentrant calls to \ref
                                               ///< ARM_CAN_MessageSend, \ref ARM_CAN_MessageRead,
                                               ///< \ref ARM_CAN_ObjectConfigure and abort message
                                               ///< sending used by \ref ARM_CAN_Control
    uint32_t fd_mode : 1;               ///< Support for CAN with flexible data-rate mode (CAN_FD)
                                        ///< (set by \ref ARM_CAN_Control)
    uint32_t restricted_mode : 1;                 ///< Support for restricted operation mode (set by
                                                  ///< \ref ARM_CAN_SetMode)
    uint32_t monitor_mode : 1;                    ///< Support for bus monitoring mode (set by \ref
                                                  ///< ARM_CAN_SetMode)
    uint32_t internal_loopback : 1;               ///< Support for internal loopback mode (set by
                                                  ///< \ref ARM_CAN_SetMode)
    uint32_t external_loopback : 1;               ///< Support for external loopback mode (set by
                                                  ///< \ref ARM_CAN_SetMode)
...
typedef struct _ARM_DRIVER_CAN {
    ...
    (void);               ///< Pointer to \ref ARM_CAN_GetCapabilities       : Get driver
                          ///< capabilities.
    int32_t (*Initialize)(
        ARM_CAN_SignalUnitEvent_t cb_unit_event,
        ARM_CAN_SignalObjectEvent_t
            cb_object_event);               ///< Pointer to \ref ARM_CAN_Initialize : Initialize CAN
                                            ///< interface.
    int32_t (*Uninitialize)(void);               ///< Pointer to \ref ARM_CAN_Uninitialize :
                                                 ///< De-initialize CAN interface.
    int32_t (*PowerControl)(
        ARM_POWER_STATE state);               ///< Pointer to \ref ARM_CAN_PowerControl          :
                                              ///< Control CAN interface power.
    uint32_t (*GetClock)(void);               ///< Pointer to \ref ARM_CAN_GetClock              :
                                              ///< Retrieve CAN base clock frequency.
    int32_t (*SetBitrate)(
        ARM_CAN_BITRATE_SELECT select, uint32_t bitrate,
        uint32_t bit_segments);               ///< Pointer to \ref ARM_CAN_SetBitrate : Set bitrate
                                              ///< for CAN interface.
    int32_t (*SetMode)(ARM_CAN_MODE mode);               ///< Pointer to \ref ARM_CAN_SetMode : Set
                                                         ///< operating mode for CAN interface.
    ARM_CAN_OBJ_CAPABILITIES (*ObjectGetCapabilities)
    (uint32_t obj_idx);               ///< Pointer to \ref ARM_CAN_ObjectGetCapabilities : Retrieve
                                      ///< capabilities of an object.
    int32_t (*ObjectSetFilter)(
        uint32_t obj_idx, ARM_CAN_FILTER_OPERATION operation, uint32_t id,
        uint32_t arg);                ///< Pointer to \ref ARM_CAN_ObjectSetFilter       : Add or
                                      ///< remove filter for message reception.
    int32_t (*ObjectConfigure)(
        uint32_t obj_idx,
        ARM_CAN_OBJ_CONFIG obj_cfg);               ///< Pointer to \ref ARM_CAN_ObjectConfigure :
                                                   ///< Configure object.
    int32_t (*MessageSend)(uint32_t obj_idx, ARM_CAN_MSG_INFO * msg_info, const uint8_t * data,
                           uint8_t size);               ///< Pointer to \ref ARM_CAN_MessageSend :
                                                        ///< Send message on CAN bus.
    int32_t (*MessageRead)(uint32_t obj_idx, ARM_CAN_MSG_INFO * msg_info, uint8_t * data,
                           uint8_t size);               ///< Pointer to \ref ARM_CAN_MessageRead :
                                                        ///< Read message received on CAN bus.
    int32_t (*Control)(uint32_t control,
                       uint32_t arg);               ///< Pointer to \ref ARM_CAN_Control : Control
                                                    ///< CAN interface.
