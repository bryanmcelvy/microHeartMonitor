...
#define ARM_ETH_PHY_SPEED_10M                                                                      \
    (ARM_ETH_SPEED_10M << ARM_ETH_PHY_SPEED_Pos)                  ///< 10 Mbps link speed
#define ARM_ETH_PHY_SPEED_100M                                                                     \
    (ARM_ETH_SPEED_100M << ARM_ETH_PHY_SPEED_Pos)                 ///< 100 Mbps link speed
#define ARM_ETH_PHY_SPEED_1G                                                                       \
    (ARM_ETH_SPEED_1G << ARM_ETH_PHY_SPEED_Pos)                   ///< 1 Gpbs link speed
#define ARM_ETH_PHY_DUPLEX_Pos 2
#define ARM_ETH_PHY_DUPLEX_Msk (1UL << ARM_ETH_PHY_DUPLEX_Pos)
#define ARM_ETH_PHY_DUPLEX_HALF                                                                    \
    (ARM_ETH_DUPLEX_HALF << ARM_ETH_PHY_DUPLEX_Pos)               ///< Half duplex link
#define ARM_ETH_PHY_DUPLEX_FULL                                                                    \
    (ARM_ETH_DUPLEX_FULL << ARM_ETH_PHY_DUPLEX_Pos)               ///< Full duplex link
...
typedef int32_t (*ARM_ETH_PHY_Read_t)(
    ...
    uint16_t *
        data);               ///< Pointer to \ref ARM_ETH_MAC_PHY_Read : Read Ethernet PHY Register.
typedef int32_t (*ARM_ETH_PHY_Write_t)(
    uint8_t phy_addr, uint8_t reg_addr,
    uint16_t data);               ///< Pointer to \ref ARM_ETH_MAC_PHY_Write : Write Ethernet PHY
                                  ///< Register.
...
typedef struct _ARM_DRIVER_ETH_PHY {
    ...
    int32_t (*Initialize)(
        ...
        ARM_ETH_PHY_Write_t fn_write);               ///< Pointer to \ref ARM_ETH_PHY_Initialize :
                                                     ///< Initialize PHY Device.
    int32_t (*Uninitialize)(void);                   ///< Pointer to \ref ARM_ETH_PHY_Uninitialize :
                                                     ///< De-initialize PHY Device.
    int32_t (*PowerControl)(
        ARM_POWER_STATE state);                      ///< Pointer to \ref ARM_ETH_PHY_PowerControl :
                                                     ///< Control PHY Device Power.
    int32_t (*SetInterface)(
        uint32_t interface);               ///< Pointer to \ref ARM_ETH_PHY_SetInterface : Set
                                           ///< Ethernet Media Interface.
    int32_t (*SetMode)(uint32_t mode);               ///< Pointer to \ref ARM_ETH_PHY_SetMode : Set
                                                     ///< Ethernet PHY Device Operation mode.
    ARM_ETH_LINK_STATE (*GetLinkState)
    (void);               ///< Pointer to \ref ARM_ETH_PHY_GetLinkState : Get Ethernet PHY Device
                          ///< Link state.
    ARM_ETH_LINK_INFO (*GetLinkInfo)
    (void);               ///< Pointer to \ref ARM_ETH_PHY_GetLinkInfo : Get Ethernet PHY Device
                          ///< Link information.
