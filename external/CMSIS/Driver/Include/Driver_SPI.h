...
#define ARM_SPI_MODE_MASTER                                                                        \
    (0x01UL << ARM_SPI_CONTROL_Pos)               ///< SPI Master (Output on MOSI, Input on MISO);
                                                  ///< arg = Bus Speed in bps
#define ARM_SPI_MODE_SLAVE                                                                         \
    (0x02UL << ARM_SPI_CONTROL_Pos)               ///< SPI Slave  (Output on MISO, Input on MOSI)
#define ARM_SPI_MODE_MASTER_SIMPLEX                                                                \
    (0x03UL                                                                                        \
     << ARM_SPI_CONTROL_Pos)               ///< SPI Master (Output/Input on MOSI); arg = Bus Speed
                                           ///< in bps @deprecated Simplex Mode has been removed
#define ARM_SPI_MODE_SLAVE_SIMPLEX                                                                 \
    (0x04UL << ARM_SPI_CONTROL_Pos)               ///< SPI Slave  (Output/Input on MISO) @deprecated
                                                  ///< Simplex Mode has been removed
...
#define ARM_SPI_CPOL0_CPHA0                                                                        \
    (0UL << ARM_SPI_FRAME_FORMAT_Pos)               ///< Clock Polarity 0, Clock Phase 0 (default)
#define ARM_SPI_CPOL0_CPHA1                                                                        \
    (1UL << ARM_SPI_FRAME_FORMAT_Pos)               ///< Clock Polarity 0, Clock Phase 1
#define ARM_SPI_CPOL1_CPHA0                                                                        \
    (2UL << ARM_SPI_FRAME_FORMAT_Pos)               ///< Clock Polarity 1, Clock Phase 0
#define ARM_SPI_CPOL1_CPHA1                                                                        \
    (3UL << ARM_SPI_FRAME_FORMAT_Pos)               ///< Clock Polarity 1, Clock Phase 1
#define ARM_SPI_TI_SSI                                                                             \
    (4UL << ARM_SPI_FRAME_FORMAT_Pos)               ///< Texas Instruments Frame Format
#define ARM_SPI_MICROWIRE                                                                          \
    (5UL                                                                                           \
     << ARM_SPI_FRAME_FORMAT_Pos)               ///< National Semiconductor Microwire Frame Format
...
#define ARM_SPI_DATA_BITS(n)                                                                       \
    (((n) &0x3FUL) << ARM_SPI_DATA_BITS_Pos)               ///< Number of Data bits
...
#define ARM_SPI_MSB_LSB                                                                            \
    (0UL << ARM_SPI_BIT_ORDER_Pos)               ///< SPI Bit order from MSB to LSB (default)
#define ARM_SPI_LSB_MSB                                                                            \
    (1UL << ARM_SPI_BIT_ORDER_Pos)               ///< SPI Bit order from LSB to MSB
...
#define ARM_SPI_SS_MASTER_UNUSED                                                                   \
    (0UL << ARM_SPI_SS_MASTER_MODE_Pos)               ///< SPI Slave Select when Master: Not used
                                                      ///< (default)
#define ARM_SPI_SS_MASTER_SW                                                                       \
    (1UL << ARM_SPI_SS_MASTER_MODE_Pos)               ///< SPI Slave Select when Master: Software
                                                      ///< controlled
#define ARM_SPI_SS_MASTER_HW_OUTPUT                                                                \
    (2UL << ARM_SPI_SS_MASTER_MODE_Pos)               ///< SPI Slave Select when Master: Hardware
                                                      ///< controlled Output
#define ARM_SPI_SS_MASTER_HW_INPUT                                                                 \
    (3UL << ARM_SPI_SS_MASTER_MODE_Pos)               ///< SPI Slave Select when Master: Hardware
                                                      ///< monitored Input
...
#define ARM_SPI_SS_SLAVE_HW                                                                        \
    (0UL << ARM_SPI_SS_SLAVE_MODE_Pos)                ///< SPI Slave Select when Slave: Hardware
                                                      ///< monitored (default)
#define ARM_SPI_SS_SLAVE_SW                                                                        \
    (1UL << ARM_SPI_SS_SLAVE_MODE_Pos)                ///< SPI Slave Select when Slave: Software
                                                      ///< controlled

/*----- SPI Control Codes: Miscellaneous Controls  -----*/
#define ARM_SPI_SET_BUS_SPEED                                                                      \
    (0x10UL << ARM_SPI_CONTROL_Pos)               ///< Set Bus Speed in bps; arg = value
#define ARM_SPI_GET_BUS_SPEED                                                                      \
    (0x11UL << ARM_SPI_CONTROL_Pos)               ///< Get Bus Speed in bps
#define ARM_SPI_SET_DEFAULT_TX_VALUE                                                               \
    (0x12UL << ARM_SPI_CONTROL_Pos)               ///< Set default Transmit value; arg = value
#define ARM_SPI_CONTROL_SS                                                                         \
    (0x13UL                                                                                        \
     << ARM_SPI_CONTROL_Pos)               ///< Control Slave Select; arg: 0=inactive, 1=active
#define ARM_SPI_ABORT_TRANSFER                                                                     \
    (0x14UL << ARM_SPI_CONTROL_Pos)               ///< Abort current data transfer
...
typedef struct _ARM_SPI_STATUS {
    ...
    uint32_t mode_fault : 1;               ///< Mode fault detected; optional (cleared on start of
                                           ///< transfer operation)
...
typedef void (*ARM_SPI_SignalEvent_t)(
    uint32_t event);               ///< Pointer to \ref ARM_SPI_SignalEvent : Signal SPI Event.

/**
\brief SPI Driver Capabilities.
*/
typedef struct _ARM_SPI_CAPABILITIES {
    uint32_t simplex : 1;                 ///< supports Simplex Mode (Master and Slave) @deprecated
...
typedef struct _ARM_DRIVER_SPI {
    ...
    int32_t (*Initialize)(
        ARM_SPI_SignalEvent_t cb_event);               ///< Pointer to \ref ARM_SPI_Initialize :
                                                       ///< Initialize SPI Interface.
    int32_t (*Uninitialize)(void);                     ///< Pointer to \ref ARM_SPI_Uninitialize :
                                                       ///< De-initialize SPI Interface.
    int32_t (*PowerControl)(
        ARM_POWER_STATE state);                  ///< Pointer to \ref ARM_SPI_PowerControl : Control
                                                 ///< SPI Interface Power.
    int32_t (*Send)(const void * data,
                    uint32_t num);               ///< Pointer to \ref ARM_SPI_Send : Start sending
                                                 ///< data to SPI Interface.
    int32_t (*Receive)(void * data,
                       uint32_t num);                ///< Pointer to \ref ARM_SPI_Receive : Start
                                                     ///< receiving data from SPI Interface.
    int32_t (*Transfer)(const void * data_out, void * data_in,
                        uint32_t num);               ///< Pointer to \ref ARM_SPI_Transfer : Start
                                                     ///< sending/receiving data to/from SPI.
