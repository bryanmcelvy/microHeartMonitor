...
typedef struct _ARM_I2C_STATUS {
    ...
    uint32_t general_call : 1;               ///< General Call indication (cleared on start of next
                                             ///< Slave operation)
    uint32_t arbitration_lost : 1;               ///< Master lost arbitration (cleared on start of
                                                 ///< next Master operation)
...
/****** I2C Event *****/
#define ARM_I2C_EVENT_TRANSFER_DONE                                                                \
    (1UL << 0)               ///< Master/Slave Transmit/Receive finished
#define ARM_I2C_EVENT_TRANSFER_INCOMPLETE                                                          \
    (1UL << 1)               ///< Master/Slave Transmit/Receive incomplete transfer
#define ARM_I2C_EVENT_SLAVE_TRANSMIT                                                               \
    (1UL << 2)               ///< Addressed as Slave Transmitter but transmit operation is not set.
#define ARM_I2C_EVENT_SLAVE_RECEIVE                                                                \
    (1UL << 3)               ///< Addressed as Slave Receiver but receive operation is not set.
#define ARM_I2C_EVENT_ADDRESS_NACK (1UL << 4)               ///< Address not acknowledged from Slave
#define ARM_I2C_EVENT_GENERAL_CALL                                                                 \
    (1UL << 5)               ///< Slave addressed with general call address
#define ARM_I2C_EVENT_ARBITRATION_LOST (1UL << 6)               ///< Master lost arbitration
#define ARM_I2C_EVENT_BUS_ERROR                                                                    \
    (1UL << 7)               ///< Bus error detected (START/STOP at illegal position)
...
/**
  ...
  \fn          int32_t ARM_I2C_MasterTransmit (uint32_t addr, const uint8_t *data, uint32_t num,
  bool xfer_pending) \brief       Start transmitting data as I2C Master. \param[in]   addr Slave
  address (7-bit or 10-bit) \param[in]   data          Pointer to buffer with data to transmit to
  I2C Slave \param[in]   num           Number of data bytes to transmit \param[in]   xfer_pending
  Transfer operation is pending - Stop condition will not be generated \return      \ref
  execution_status

  \fn          int32_t ARM_I2C_MasterReceive (uint32_t addr, uint8_t *data, uint32_t num, bool
  xfer_pending) \brief       Start receiving data as I2C Master. \param[in]   addr          Slave
  address (7-bit or 10-bit) \param[out]  data          Pointer to buffer for data to receive from
  I2C Slave \param[in]   num           Number of data bytes to receive \param[in]   xfer_pending
  Transfer operation is pending - Stop condition will not be generated \return      \ref
  execution_status
...
typedef void (*ARM_I2C_SignalEvent_t)(
    uint32_t event);               ///< Pointer to \ref ARM_I2C_SignalEvent : Signal I2C Event.
...
typedef struct _ARM_DRIVER_I2C {
    ...
    int32_t (*Initialize)(
        ARM_I2C_SignalEvent_t cb_event);               ///< Pointer to \ref ARM_I2C_Initialize :
                                                       ///< Initialize I2C Interface.
    int32_t (*Uninitialize)(void);                     ///< Pointer to \ref ARM_I2C_Uninitialize :
                                                       ///< De-initialize I2C Interface.
    int32_t (*PowerControl)(
        ARM_POWER_STATE state);               ///< Pointer to \ref ARM_I2C_PowerControl : Control
                                              ///< I2C Interface Power.
    int32_t (*MasterTransmit)(
        uint32_t addr, const uint8_t * data, uint32_t num,
        bool xfer_pending);                   ///< Pointer to \ref ARM_I2C_MasterTransmit : Start
                                              ///< transmitting data as I2C Master.
    int32_t (*MasterReceive)(
        uint32_t addr, uint8_t * data, uint32_t num,
        bool xfer_pending);                   ///< Pointer to \ref ARM_I2C_MasterReceive : Start
                                              ///< receiving data as I2C Master.
    int32_t (*SlaveTransmit)(
        const uint8_t * data,
        uint32_t num);               ///< Pointer to \ref ARM_I2C_SlaveTransmit : Start transmitting
                                     ///< data as I2C Slave.
    int32_t (*SlaveReceive)(uint8_t * data,
                            uint32_t num);               ///< Pointer to \ref ARM_I2C_SlaveReceive :
