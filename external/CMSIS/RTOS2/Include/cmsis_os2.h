...
typedef enum {
    ...
    osErrorNoMemory = -5,                ///< System is out of memory: it was impossible to allocate
                                         ///< or reserve memory for the operation.
    osErrorISR = -6,               ///< Not allowed in ISR context: the function cannot be called
...
/// \param[in]     ticks         \ref kernelTimer "time ticks" value until the thread watchdog
/// expires, or 0 to stop the watchdog \return status code that indicates the execution status of
/// the function.
