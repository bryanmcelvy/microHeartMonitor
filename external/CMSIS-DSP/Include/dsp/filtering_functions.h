...
typedef struct {
    uint16_t numTaps;   /**< number of filter coefficients in the filter. */
    float64_t * pState; /**< points to the state variable array. The array is of length
                           numTaps+blockSize-1. */
    const float64_t *
        pCoeffs;        /**< points to the coefficient array. The array is of length numTaps. */
...
/**
  ...
  @param[in]     pComputedCoeffs   points to the new computed coefficients for the vectorized
  version.
  @return        none
*/
 ...
 * @param[in]  pScratch2  points to scratch buffer (of type q15_t) of size min(srcALen, srcBLen).
 */
void arm_correlate_opt_q7(const q7_t * pSrcA, uint32_t srcALen, const q7_t * pSrcB,
