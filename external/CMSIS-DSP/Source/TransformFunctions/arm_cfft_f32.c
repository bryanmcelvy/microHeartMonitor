...
static void _arm_radix4_butterfly_f32_mve(const arm_cfft_instance_f32 * S, float32_t * pSrc,
                                          uint32_t fftLen) {
...
static void arm_cfft_radix4by2_f32_mve(const arm_cfft_instance_f32 * S, float32_t * pSrc,
                                       uint32_t fftLen) {
...
static void _arm_radix4_butterfly_inverse_f32_mve(const arm_cfft_instance_f32 * S, float32_t * pSrc,
                                                  uint32_t fftLen, float32_t onebyfftLen) {
    f32x4_t vecTmp0, vecTmp1;
    f32x4_t vecSum0, vecDiff0, vecSum1, vecDiff1;
    f32x4_t vecA, vecB, vecC, vecD;
    uint32_t blkCnt;
...
static void arm_cfft_radix4by2_inverse_f32_mve(const arm_cfft_instance_f32 * S, float32_t * pSrc,
                                               uint32_t fftLen) {
    float32_t const * pCoefVec;
    float32_t const * pCoef = S->pTwiddle;
    float32_t *pIn0, *pIn1;
...
void arm_cfft_f32(const arm_cfft_instance_f32 * S, float32_t * pSrc, uint8_t ifftFlag,
...
/**
  ...
  @par
                   ...
                   <b>mandatory</b>. <b>Compilation flags are available to include only the required
  tables for the needed FFTs.</b> Other FFT versions can continue to be initialized as explained
  below.
  @par
  ...
  <code>arm_const_structs.h</code>.  Include this header in your function and then pass one of the
  constant structures as an argument to arm_cfft_f32.  For example:
  @par
  ...
  @par
                   ...
                   than the new functions.
  @par
                   An example of initialization of the constants for the arm_cfft_q31 function
  follows:
  @code
