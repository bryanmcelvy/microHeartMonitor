...
/**
  ...
  multiplications yield a 2.14 result. The 2.14 intermediate results are accumulated in a 32-bit
  accumulator in 18.14 format. There is no risk of internal overflow with this approach and the full
  precision of intermediate multiplications is preserved. The accumulator is converted to 18.7
  format by discarding the low 7 bits. Finally, the result is truncated to 1.7 format.
 */
...
static void arm_fir_q7_49_64_mve(const arm_fir_instance_q7 * S, const q7_t * __restrict pSrc,
...
void arm_fir_q7_33_48_mve(const arm_fir_instance_q7 * S, const q7_t * __restrict pSrc,
                          q7_t * __restrict pDst, uint32_t blockSize) {
#define NBTAPS    48
#define NBVECTAPS (NBTAPS / 16)
    FIR_Q7_MAIN_CORE();
...
static void arm_fir_q7_17_32_mve(const arm_fir_instance_q7 * S, const q7_t * __restrict pSrc,
                                 q7_t * __restrict pDst, uint32_t blockSize) {
#define NBTAPS    32
#define NBVECTAPS (NBTAPS / 16)
    FIR_Q7_MAIN_CORE();
...
void arm_fir_q7_1_16_mve(const arm_fir_instance_q7 * S, const q7_t * __restrict pSrc,
                         q7_t * __restrict pDst, uint32_t blockSize) {
#define NBTAPS    16
#define NBVECTAPS (NBTAPS / 16)
    FIR_Q7_MAIN_CORE();
