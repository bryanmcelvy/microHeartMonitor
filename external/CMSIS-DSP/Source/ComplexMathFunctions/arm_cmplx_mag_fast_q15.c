...
/**
  ...
  converted into 2.14 format. Fast functions are less accurate. This function will tend to clamp to
  0 the too small values. So sqrt(x*x) = x will not always be true.
 */
...
void arm_cmplx_mag_fast_q15(const q15_t * pSrc, q15_t * pDst, uint32_t numSamples) {
    ...
    while(blkCnt > 0U) {
        vecSrc = vld2q(pSrc);
        pSrc += 16;
        sum = vqaddq(vmulhq(vecSrc.val[0], vecSrc.val[0]), vmulhq(vecSrc.val[1], vecSrc.val[1]));
