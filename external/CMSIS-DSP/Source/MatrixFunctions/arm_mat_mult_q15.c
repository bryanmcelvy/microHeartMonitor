...
/**
  ...
  @par           Scaling and Overflow Behavior
                   The function is implemented using an internal 64-bit accumulator. The inputs to
  the multiplications are in 1.15 format and multiplications yield a 2.30 result. The 2.30
  intermediate results are accumulated in a 64-bit accumulator in 34.30 format. This approach
  provides 33 guard bits and there is no risk of overflow. The 34.30 result is then truncated
  to 34.15 format by discarding the low 15 bits and then saturated to 1.15 format.
  @par
                   Refer to \ref arm_mat_mult_fast_q15() for a faster but less precise version of
  this function.
...
__STATIC_INLINE arm_status arm_mat_mult_q15_2x2_mve(const arm_matrix_instance_q15 * pSrcA,
...
__STATIC_INLINE arm_status arm_mat_mult_q15_3x3_mve(const arm_matrix_instance_q15 * pSrcA,
                                                    const arm_matrix_instance_q15 * pSrcB,
                                                    arm_matrix_instance_q15 * pDst) {
    q15_t * pInB = pSrcB->pData; /* input data matrix pointer B */
    q15_t * pInA = pSrcA->pData; /* input data matrix pointer A */
...
__STATIC_INLINE arm_status arm_mat_mult_q15_4x4_mve(const arm_matrix_instance_q15 * pSrcA,
                                                    const arm_matrix_instance_q15 * pSrcB,
                                                    arm_matrix_instance_q15 * pDst) {
    q15_t * pInB = pSrcB->pData; /* input data matrix pointer B */
    q15_t * pInA = pSrcA->pData; /* input data matrix pointer A */
...
arm_status arm_mat_mult_q15(const arm_matrix_instance_q15 * pSrcA,
...
#endif                    /* #ifdef ARM_MATH_MATRIX_CHECK */
    {
        ...
        do {
            ...
            /* For every row wise process, pIn2 pointer is set to starting address of transposed
             * pSrcB data */
            pInB = pSrcBT;
...

    {
        ...
        do {
            ...
            do {
                ...
                /* Convert result from 34.30 to 1.15 format and store saturated value in destination
                 * buffer */
