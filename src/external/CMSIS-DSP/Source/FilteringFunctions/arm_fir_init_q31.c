...
/**
  ...
  reversed order: <pre> {b[numTaps-1], b[numTaps-2], b[N-2], ..., b[1], b[0]}
                   ...
                   <code>pState</code> is of length <code>numTaps+blockSize-1</code> samples (except
  for Helium - see below), where <code>blockSize</code> is the number of input samples processed by
  each call to <code>arm_fir_q31()</code>.
  ...
  to manage too many different cases in the code.
    ...
    @par          Helium state buffer
                   ...
                   So the state buffer has size <code> numTaps + 8*ceil(blockSize/4) + blockSize - 1
  </code>
