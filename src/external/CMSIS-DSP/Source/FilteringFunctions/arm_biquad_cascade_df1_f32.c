...
/**
  ...
  2, and so on. The state array has a total length of <code>4*numStages</code> values. The state
  ...
  arrays cannot be shared. There are separate instance structure declarations for each of the 3
  ...
  to zeros before static initialization. The code below statically initializes each of the 3
  different data type filter instance structures <pre> arm_biquad_casd_df1_inst_f32 S1 = {numStages,
  pState, pCoeffs}; arm_biquad_casd_df1_inst_q15 S2 = {numStages, pState, pCoeffs, postShift};
  ...
  <code>postShift</code> which allow the filter coefficients to exceed the range <code>[+1
  ...
  filter increases the amplitude of certain frequencies. This means that an input signal with
  amplitude < 1.0 may result in an output > 1.0 and these are saturated or overflowed based on the
  implementation of the filter. To avoid this behavior the filter needs to be scaled down such that
  its peak gain < 1.0 or the input signal must be scaled down so that the combination of input and
  filter are never overflowed.
    ...
    while(stage > 0U) {
        ...
        while(sample > 0U) {
            ...
            acc = (vgetq_lane_f32(b, 1) * vgetq_lane_f32(Xn, 2)) +
                  (vgetq_lane_f32(b, 2) * vgetq_lane_f32(Xn, 3)) + (vgetq_lane_f32(b, 3) * Xns) +
