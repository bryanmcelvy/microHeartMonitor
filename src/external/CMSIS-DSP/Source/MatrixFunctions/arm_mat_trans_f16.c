...
arm_status arm_mat_trans_f16(const arm_matrix_instance_f16 * pSrc, arm_matrix_instance_f16 * pDst) {
...

    {
        if(pDst->numRows == pDst->numCols) {
            ...
            if(pDst->numCols == 3)
                return arm_mat_trans_16bit_3x3_mve((uint16_t *) pSrc->pData,
                                                   (uint16_t *) pDst->pData);
            if(pDst->numCols == 4)
                return arm_mat_trans_16bit_4x4_mve((uint16_t *) pSrc->pData,
                                                   (uint16_t *) pDst->pData);
        }

        arm_mat_trans_16bit_generic(pSrc->numRows, pSrc->numCols, (uint16_t *) pSrc->pData,
                                    (uint16_t *) pDst->pData);
