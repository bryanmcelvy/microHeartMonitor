...
/**
   ...
   * @param[out] pl   points to the instance of the output floating-point triangular matrix
   structure.
   ...
   */
arm_status arm_mat_ldlt_f32(const arm_matrix_instance_f32 * pSrc, arm_matrix_instance_f32 * pl,
                            arm_matrix_instance_f32 * pd, uint16_t * pp) {
        ...
        for(k = 0; k < n; k++) {
            ...
            for(w = k + 1; w < n; w += 4) {
                ...
                for(int x = k + 1; x < n; x += 4) {
                    ...
                    vecX = vldrwq_gather_shifted_offset_z_f32(&pA[x * n + k], (uint32x4_t) vecOffs,
                                                              p0);
