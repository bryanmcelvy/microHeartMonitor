...
static uint32_t arm_quick_sort_partition_f32(float32_t * pSrc, int32_t first, int32_t last,
                                             uint8_t dir) {
    ...
    while(i < j) {
         ...
         * For example, in the extreme case of an ordered array the do-while loop related to i will
         * stop at the first iteration (because pSrc[i]=pSrc[pivot] already), and the loop related
         * to j will stop after (last-first) iterations (when j=pivot=i=first). j is returned and
