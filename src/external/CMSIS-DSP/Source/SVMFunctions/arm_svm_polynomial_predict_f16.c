...
void arm_svm_polynomial_predict_f16(const arm_svm_polynomial_instance_f16 * S, const float16_t * in,
    ...
    while(row >= 4) {
        ...
        vSum = vfmaq_m_f16(
            vSum, vld1q(pDualCoef),
            arm_vec_exponent_f16(vaddq_n_f16(vmulq_n_f16(vtmp, S->gamma), S->coef0), S->degree),
            vctp16q(4));
...
void arm_svm_polynomial_predict_f16(const arm_svm_polynomial_instance_f16 * S, const float16_t * in,
    ...
    for(i = 0; i < S->nbOfSupportVectors; i++) {
        ...
        sum += (_Float16) S->dualCoefficients[i] *
               (_Float16) arm_exponent_f16(
                   (_Float16) S->gamma * (_Float16) dot + (_Float16) S->coef0, S->degree);
    }
