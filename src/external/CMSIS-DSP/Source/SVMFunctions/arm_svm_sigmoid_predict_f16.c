...
void arm_svm_sigmoid_predict_f16(const arm_svm_sigmoid_instance_f16 * S, const float16_t * in,
    ...
    if(row >= 2) {
        ...
        vSum =
            vfmaq_m_f16(vSum, vld1q(pDualCoef),
                        vtanhq_f16(vaddq_n_f16(vmulq_n_f16(vtmp, S->gamma), S->coef0)), vctp16q(2));
    ...
    if(row >= 1) {
        ...
        vSum =
            vfmaq_m_f16(vSum, vld1q(pDualCoef),
                        vtanhq_f16(vaddq_n_f16(vmulq_n_f16(vtmp, S->gamma), S->coef0)), vctp16q(1));
    }
...
void arm_svm_sigmoid_predict_f16(const arm_svm_sigmoid_instance_f16 * S, const float16_t * in,
    ...
    for(i = 0; i < S->nbOfSupportVectors; i++) {
        ...
        sum += (_Float16) S->dualCoefficients[i] *
               (_Float16) tanhf(
                   (float32_t) ((_Float16) S->gamma * (_Float16) dot + (_Float16) S->coef0));
    }
    *pResult = S->classes[STEP(sum)];
}
